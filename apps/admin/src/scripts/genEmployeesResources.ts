...
import { resources } from './employeeResourcesConfig';
...
const genResource = (item: Resource) => {
	...
	const { code } = shell.exec(
		`yarn gen r employee_resource --args ${resource} ${intent} ${recordRepresentation} ${icon} ${hasCreate} ${hasEdit} ${hasShow} ${path}`,
	);
